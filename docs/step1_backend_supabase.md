...
@app.get("/health")
def health():
    # 요청마다 새로 연결한다. 지금은 이게 제일 단순해서 이렇게 두고,
    # 테이블이 생기는 시점에 커넥션 풀로 바꾼다.
...
> 📌 **알면서 남겨둔 부분:** `/health`는 요청이 올 때마다 DB에 **새로 접속**합니다. 원래는 접속을 미리 몇 개 만들어놓고 재사용(커넥션 풀)해야 하지만, 지금은 코드가 단순한 게 더 중요해서 이렇게 뒀습니다. 테이블이 생기는 Step 2에서 고칩니다.
