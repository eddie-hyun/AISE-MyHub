...
> 💡 **AI 없이 직접 만들어도 됩니다.** 위 세 파일을 손으로 만들고 `main.py`에 **1-5장의 최종 코드**를 그대로 옮겨 적으면 결과는 같습니다.

> 📌 **AI가 만든 초안은 다를 수 있습니다.** 이 시점의 `main.py`는 변수 이름과 에러 처리 방식이 지금의 1-5장에서 나온 코드와 다를 수 있습니다. 2~6단계를 진행하며 접속 문제(트러블슈팅 ④)를 거치며 다듬어진 것입니다. 지금 나온 코드가 1-5장과 다르게 생겼어도 정상입니다.
