...
그리고 **CORS도 안 생깁니다.** 휴대폰 브라우저 입장에서도 모든 요청이 `192.168.0.12:5173` 한 곳으로 가니까요 — 5.7의 프록시 구조가 여기서도 그대로 통합니다.
...
이 단계의 핵심 개념이라 따로 다룹니다.

### 5.1 가장 흔한 오해부터

> ❌ "CORS는 서버가 아무나 못 부르게 막는 보안 기능이다"

**아닙니다.** 서버는 여전히 누구나 부를 수 있습니다.
...
> ✅ **CORS는 브라우저 안에서만 작동하는 규칙이고, 서버가 아니라 "사용자"를 보호합니다.**

### 5.2 그럼 뭘 막는 건가

1. 당신이 은행 사이트에 로그인합니다 → 브라우저에 **세션 쿠키**가 저장됩니다
2. 로그아웃 안 하고, 우연히 악성 사이트 `evil.com` 에 들어갑니다
3. `evil.com` 의 자바스크립트가 몰래 `bank.com/api/내계좌` 를 호출합니다
4. **브라우저는 쿠키를 자동으로 붙여 보냅니다** ← 여기가 핵심
5. 은행 서버는 "로그인한 사용자네" 하고 잔액을 응답합니다

여기서 브라우저가 개입합니다.

> 🛡️ **"응답이 왔지만, 너(evil.com)는 이걸 읽을 수 없다."**

**요청 자체를 막는 게 아니라 응답을 읽는 것을 막습니다.** 그래서 서버 로그에는 요청이 찍힙니다. 디버깅할 때 헷갈리는 부분이에요.

### 5.3 "다른 출처"의 정확한 기준

**스킴 · 호스트 · 포트** 세 가지가 **전부** 같아야 같은 출처입니다.

기준: `http://localhost:5174`

| 비교 대상 | 같은 출처? | 이유 |
|---|---|---|
| `http://localhost:5174/about` | ✅ | 경로는 상관없음 |
| `http://localhost:8080` | ❌ | **포트가 다름** ← 우리가 겪을 뻔한 것 |
| `https://localhost:5174` | ❌ | 스킴이 다름 (http ≠ https) |
| `http://127.0.0.1:5174` | ❌ | **글자가 다르면 다름** (같은 컴퓨터여도) |

마지막 줄이 은근히 사람 잡습니다. `localhost`와 `127.0.0.1`은 브라우저에게 **다른 이름**입니다.
...
```
Access to fetch at 'http://127.0.0.1:8080/api/profile'
...
| 조각 | 뜻 |
|---|---|
| `from origin 'http://localhost:5174'` | **누가** 요청했나 |
| `at 'http://127.0.0.1:8080/...'` | **어디로** 요청했나 |
| `No 'Access-Control-Allow-Origin' header` | 서버가 **"허락한다"는 응답 헤더를 안 줬다** |
...
FastAPI에서는 이렇게 붙입니다. **우리는 이걸 안 썼습니다.**
...
### 5.6 Preflight — Step 4에서 만나게 될 두 번째 벽

요청에 따라 브라우저가 **본 요청 전에 허락부터 물어봅니다.**

```
브라우저 ──① OPTIONS (물어보기)──▶ 서버
브라우저 ◀─② 허락한다 ────────── 서버
브라우저 ──③ POST (진짜 요청)───▶ 서버
```

| 예비 요청 없음 (단순 요청) | 예비 요청 발생 |
|---|---|
| `GET`, `HEAD`, 단순 `POST` | `PUT`, `PATCH`, `DELETE` |
| 특별한 헤더 없음 | **`Content-Type: application/json`** |
| | 인증 헤더 등 커스텀 헤더 |

**그런데 우리는 이걸 만나지 않습니다.** 프록시 덕분에 브라우저 입장에서는 모든 요청이 같은 출처(5174)로 가거든요. 같은 출처에는 CORS 검사 자체가 없으니 예비 요청도 없습니다.

프록시 없이 8080으로 직접 요청하는 구조였다면, Step 4에서 `PUT`으로 이력서를 수정하는 순간 만나게 됩니다. **"GET은 되는데 PUT만 안 되네?"** 하고 당황하는 게 전형적인 패턴이에요. 프록시가 조용히 막아준 두 번째 문제입니다.

### 5.7 해결책 3가지와 우리의 선택

| 방법 | 언제 | 비용 |
|---|---|---|
| **① 프록시** ← 우리 | 개발 중 | 설정 4줄, 백엔드 코드 0줄 |
| ② 서버에서 CORS 허용 | 프론트/백엔드를 다른 도메인에 배포할 때 | 미들웨어 + 쿠키 설정 골치 |
| ③ 같은 출처로 배포 | 운영 | 백엔드가 화면 파일까지 서빙 |

우리는 **① + ③** 조합입니다.

```
개발:  브라우저(5174) → Vite(5174) → FastAPI(8080)     프록시가 다리 역할
배포:  브라우저(내도메인) → FastAPI(내도메인)            애초에 같은 출처
```

**양쪽 다 브라우저 입장에서는 "한 곳"이라 CORS가 발생할 여지 자체가 없습니다.**

### 5.8 프록시를 고른 진짜 이유는 Step 4에 있습니다 (쿠키)

②를 택하면 **로그인 쿠키가 골치 아파집니다.**

| | 프록시 (①) | CORS 허용 (②) |
|---|---|---|
| 쿠키 설정 | `SameSite=Lax` — 기본값 | **`SameSite=None; Secure`** 필수 |
| HTTPS | 개발 중엔 불필요 | 개발 환경에도 HTTPS 필요해짐 |
| `fetch` | 그냥 호출 | `credentials: 'include'` 매번 |
| 브라우저 차단 | 없음 | 서드파티 쿠키 차단 정책에 걸릴 수 있음 |

> **쿠키 속성 미리보기** — Step 4에서 자세히 다룹니다.
>
> | 속성 | 뜻 | 왜 필요한가 |
> |---|---|---|
> | `httpOnly` | 자바스크립트가 못 읽음 | XSS로 토큰을 훔쳐가지 못하게 |
> | `Secure` | HTTPS에서만 전송 | 중간에서 가로채지 못하게 |
> | `SameSite=Lax` | **다른 사이트에서 온 요청엔 안 붙음** | 위 5.2의 은행 시나리오를 원천 차단 |
> | `SameSite=None` | 다른 사이트 요청에도 붙음 | 반드시 `Secure`와 함께 써야 함 |
>
> 프론트/백엔드가 **같은 출처**면 `SameSite=Lax` 기본값으로 끝납니다. 다른 출처면 `None; Secure`가 필요하고, 그러면 개발 환경에도 HTTPS를 붙여야 합니다.
> **로그인 기능을 붙일 때 이 차이가 하루를 날리느냐 마느냐를 가릅니다.**

### 5.9 주의 — 프록시는 개발 전용입니다

```
npm run dev     → Vite 서버가 돎 → 프록시 동작 ✅
npm run build   → 정적 파일만 생성 → 프록시 없음 ❌
```

빌드 결과물은 그냥 HTML/JS 파일 묶음이라 프록시가 따라가지 않습니다. **배포할 때는 위 ③번(같은 출처)으로 가야 합니다.** Step 5에서 다룹니다.

---

## 6. ③ Pydantic DTO와 타입 자동 생성

### 6.1 먼저 백엔드에 "응답의 모양"을 명시해야 합니다
...
| **출처 (Origin)** | 스킴 + 호스트 + 포트. 셋 중 하나만 달라도 다른 출처 |
| **CORS** | 다른 출처의 응답을 읽지 못하게 막는 **브라우저**의 규칙 |
| **Preflight** | 본 요청 전에 `OPTIONS`로 허락을 먼저 묻는 것 |
| **프록시** | 요청을 대신 받아 다른 서버로 넘겨주는 것. CORS를 우회하는 게 아니라 **발생하지 않게** 함 |
...
| | |
|---|---|
| **`POST` / `PUT` 요청** | 5.6의 **Preflight**를 실제로 만나게 됩니다 |
| **쿠키와 세션** | 5.8의 `httpOnly`, `SameSite`가 실전으로 |
